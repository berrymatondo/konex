...
const fetcher=(url:string)=>fetch(url,{cache:"no-store"}).then(async r=>{if(!r.ok){const body=await r.json().catch(()=>null);throw new Error(body?.error||"Unable to load receipts")}return r.json()})

export function BccReceiptAssaySummary(){
  const {language}=useLanguage(),fr=language==="fr",router=useRouter()
  const {data=[],error,isLoading,mutate}=useSWR<Row[]>("/api/bcc/receipts",fetcher,{revalidateOnMount:true,revalidateOnFocus:true,dedupingInterval:0})
  ...
  const rows=useMemo(()=>data.filter(r=>(statusFilter==="all"||r.status===statusFilter)&&`${r.reference} ${r.purchase_order_id} ${JSON.stringify(r.data)}`.toLowerCase().includes(search.toLowerCase())),[data,search,statusFilter])
  ...
  return <SidebarProvider><div className="flex h-screen"><AppSidebar/><div className="flex min-w-0 flex-1 flex-col overflow-hidden"><AppHeader title={fr?"Liste des réceptions":"Receipt List"} subtitle={fr?"Sommaire des réceptions physiques et résultats d’essai":"Physical receipts and assay summary"}/><main className="flex-1 overflow-y-auto p-4 md:p-6"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <Card className="overflow-hidden py-0"><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>{fr?"Référence":"Reference"}</TableHead><TableHead>{fr?"Ordre d’achat":"Purchase order"}</TableHead><TableHead>{fr?"Réception":"Receipt"}</TableHead><TableHead>{fr?"Poids net":"Net weight"}</TableHead><TableHead>{fr?"Pureté finale":"Final purity"}</TableHead><TableHead>{fr?"Statut":"Status"}</TableHead><TableHead className="w-28"/></TableRow></TableHeader><TableBody>
      {isLoading?<TableRow><TableCell colSpan={7} className="h-32 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin"/></TableCell></TableRow>:error?<TableRow><TableCell colSpan={7} className="h-32 text-center text-destructive"><p>{fr?"Impossible de charger les réceptions.":"Unable to load receipts."}</p><Button className="mt-3" variant="outline" onClick={()=>mutate()}>{fr?"Réessayer":"Retry"}</Button></TableCell></TableRow>:rows.length===0?<TableRow><TableCell colSpan={7} className="h-32 text-center text-muted-foreground">{fr?"Aucune réception enregistrée.":"No receipt recorded."}</TableCell></TableRow>:paged.map(r=>{const d=r.data||{},received=["received","confirmed"].includes(r.status),net=Number(d.netWeightKg??(Number(d.grossWeightKg||0)-Number(d.tareWeightKg||0)));return <TableRow key={r.id} className="cursor-pointer" onClick={()=>router.push(`/central-bank/receipts/receipt-assay?recordId=${r.id}&view=1`)}><TableCell className="font-mono font-semibold">{r.reference}</TableCell><TableCell className="font-mono">{r.purchase_order_id||"—"}</TableCell><TableCell>{d.receiptDate||"—"}<div className="text-xs text-muted-foreground">{d.manifestReference||"—"}</div></TableCell><TableCell>{net.toLocaleString(fr?"fr-FR":"en-US",{minimumFractionDigits:3,maximumFractionDigits:3})} kg</TableCell><TableCell>{Number(d.finalPurityPercent||0).toFixed(3)} %</TableCell><TableCell><Badge variant={received?"default":"secondary"}>{fr?(received?"Reçu":"Enregistré"):(received?"Received":"Saved")}</Badge></TableCell><TableCell><div className="flex" onClick={e=>e.stopPropagation()}>{received?<Button asChild variant="ghost" size="icon" title={fr?"Télécharger le certificat":"Download certificate"}><a href={`/api/bcc/receipt-assay/${r.id}/pdf?lang=${fr?"fr":"en"}`} download><Download className="h-4 w-4"/></a></Button>:<><Button asChild variant="ghost" size="icon" title={fr?"Modifier":"Edit"}><Link href={`/central-bank/receipts/receipt-assay?recordId=${r.id}`}><Pencil className="h-4 w-4"/></Link></Button><Button variant="ghost" size="icon" className="text-destructive" title={fr?"Supprimer":"Delete"} onClick={()=>setRemove(r)}><Trash2 className="h-4 w-4"/></Button></>}</div></TableCell></TableRow>})}
