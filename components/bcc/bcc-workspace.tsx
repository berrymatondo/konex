...
function BccTransactions() {
  ...
  const statuses = useMemo(() => Array.from(new Set(displayedOrders.map(order => order.status))).sort(), [displayedOrders])
  const rows = useMemo(() => displayedOrders.filter(order => (statusFilter === "all" || order.status === statusFilter) && `${order.reference} ${JSON.stringify(order.data)}`.toLowerCase().includes(search.toLowerCase())), [displayedOrders, search, statusFilter])
  ...
  return <div className="space-y-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-xl font-bold sm:text-2xl">{fr?"Ordres d’achat":"Purchase orders"}</h1><p className="text-sm text-muted-foreground">{fr?"Consulter les transactions et leur progression dans le cycle de vie":"Review transactions and their lifecycle progress"}</p></div><Button asChild className="w-full sm:w-auto"><Link href="/central-bank/transactions/purchase-orders?new=1"><FilePlus2 className="mr-2 h-4 w-4"/>{fr?"Nouvel ordre d’achat":"New purchase order"}</Link></Button></div>
  <div className="flex flex-col gap-3 sm:flex-row sm:items-center"><div className="relative max-w-md flex-1"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground"/><Input className="pl-9" value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}} placeholder={fr?"Rechercher…":"Search…"}/></div><BccStatusFilter value={statusFilter} onChange={v=>{setStatusFilter(v);setPage(1)}} statuses={statuses} fr={fr}/></div>
  <Card className="gap-0 py-0 overflow-hidden"><Table><TableHeader><TableRow className="bg-muted/30 hover:bg-muted/30">
    ...
    return <TableRow key={order.id} className="cursor-pointer hover:bg-muted/30" onClick={()=>router.push(`/central-bank/transactions/purchase-orders?recordId=${encodeURIComponent(order.id)}&view=1`)}><TableCell className="px-4 py-5 align-top"><div className="font-mono font-semibold text-foreground">{order.reference}</div><div className="mt-1 max-w-[160px] whitespace-normal text-xs text-muted-foreground">{String(d.contractReference||"—")}</div></TableCell>
      ...
      <TableCell className="px-4 py-5 align-top font-mono font-medium">{Number(d.targetKg||0).toLocaleString(fr?"fr-FR":"en-US",{minimumFractionDigits:3,maximumFractionDigits:3})}<div>kg</div></TableCell>
