...
export function RefiningOrderDispatch() {
  ...
  const [dispatchWeight, setDispatchWeight] = useState("50.000");
  ...
  const variance = (Number(dispatchWeight) || 0) - LOT_GROSS;
  const weightMatches = Math.abs(variance) < 0.0005;
  const dualControlComplete = dispatchingSigned && witnessSigned;
  const canConfirm = dualControlComplete && !confirmed;
  const insuredValue = FINE_KG * OZ_PER_KG * PRICE;
  ...
  return (
    <div className="space-y-5">
      <Link href="/refining-orders/GAC-REF-2026-014/approval" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" />{fr ? "Retour à l’ordre de raffinage GAC-REF-2026-014" : "Back to refining order GAC-REF-2026-014"}</Link>
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div><div className="flex flex-wrap items-center gap-2"><h1 className="font-mono text-xl font-semibold">GAC-DSP-2026-014</h1><StatusPill tone={confirmed ? "success" : "info"}>{confirmed ? (fr ? "Expédié" : "Dispatched") : (fr ? "Approuvé · prêt à expédier" : "Approved · ready to dispatch")}</StatusPill><StatusPill>{fr ? "Double contrôle" : "Dual control"}</StatusPill>{saved && <StatusPill tone="success">{fr ? "Enregistré" : "Saved"}</StatusPill>}</div><p className="mt-1 text-xs text-muted-foreground">GAC-REF-2026-014 · PO GAC-TRK-MRUYZ7EK · Lot DORE-2026-0421</p></div>
      ...
      <WorkflowStepper active={confirmed ? 3 : 2} hrefs={["/refining-orders", "/refining-orders/GAC-REF-2026-014/approval", "/refining-orders/GAC-REF-2026-014/dispatch", undefined, undefined, undefined, "/refining-orders/GAC-REF-2026-014/reserve-eligibility"]} labels={fr ? ["Brouillon", "Approuvé", "Expédition", "En raffinage", "Outturn reçu", "Rapproché", "Classification"] : ["Draft", "Approved", "Dispatch", "In refining", "Outturn received", "Reconciled", "Classification"]} />
      ...
      <Tabs defaultValue="details">
        ...
        <TabsContent value="details" className="mt-4 space-y-4">
          ...
          <RefiningPanel icon={LockKeyhole} title={fr ? "Sécurité et scellés inviolables" : "Security & tamper-evident seals"}>
            ...
            <Label>{fr ? "Numéros de scellés" : "Seal numbers"} <span className="text-primary">*</span></Label>
          ...
          <RefiningPanel icon={Users} title={<>{fr ? "Signature en double contrôle" : "Dual-control sign-off"} <span className="text-primary">*</span></>}>
            ...
            <div className="mt-3 rounded-lg border border-l-4 border-l-primary bg-primary/5 p-4 text-xs text-muted-foreground"><strong className="text-foreground">{fr ? "Chaîne de garde (US-R03)." : "Chain of custody (US-R03)."}</strong> {fr ? "Chaque remise est documentée : poids de sortie vérifié, scellés posés et constatés sous double contrôle, transit assuré, puis rapprochement à la réception. Les 44.250 kg d’or fin constituent la référence de l’outturn US-R04." : "Every handoff is evidenced: verified weight out, seals witnessed under dual control, insured transit, then matched receipt. The 44.250 kg fine gold becomes the US-R04 outturn baseline."}</div>
          </RefiningPanel>

          <div className="flex flex-col gap-3 border-t pt-5 sm:flex-row sm:items-center sm:justify-between"><p className={`text-xs ${dualControlComplete ? "text-emerald-500" : "text-muted-foreground"}`}>{confirmed ? (fr ? "Expédition confirmée. Réception de la raffinerie en attente." : "Dispatch confirmed. Refiner receipt is pending.") : dualControlComplete ? (fr ? "Double contrôle terminé — prêt à confirmer." : "Dual control complete — ready to confirm dispatch.") : (fr ? "Les deux signatures sont nécessaires pour confirmer l’expédition." : "Both sign-offs are required to confirm dispatch.")}</p><div className="flex justify-end gap-2"><Button variant="ghost" asChild><Link href="/refining-orders/GAC-REF-2026-014/approval">{fr ? "Annuler" : "Cancel"}</Link></Button><Button variant="outline" onClick={() => setSaved(true)} disabled={confirmed}>{fr ? "Enregistrer" : "Save Draft"}</Button><Button onClick={confirmDispatch} disabled={!canConfirm}><Truck className="mr-2 h-4 w-4" />{fr ? "Confirmer l’expédition" : "Confirm dispatch"}</Button></div></div>
        </TabsContent>
        <TabsContent value="trace" className="mt-4"><RefiningPanel icon={Box} title={fr ? "Chaîne de garde et de provenance" : "Custody & provenance chain"}><Timeline items={[
          ...
          { state: "done", title: "Refining order created & approved — GAC-REF-2026-014", meta: "22/07/2026 · dual approval complete (US-R02)" },
          { state: confirmed ? "done" : "current", title: "Dispatch & chain of custody — GAC-DSP-2026-014", meta: confirmed ? (fr ? "Expédition confirmée" : "Dispatch confirmed") : (fr ? "Cette étape · scellés, poids, transport et assurance (US-R03)" : "This step · seals, weight, transport & insurance (US-R03)") },
...
function Field({ label, required, children }: { label: string; required?: boolean; children: ReactNode }) {
  return <div className="space-y-2"><Label>{label}{required && <span className="ml-1 text-primary">*</span>}</Label>{children}</div>;
}
