...
export function RefiningOrderDispatch() {
  ...
  return (
    <div className="space-y-5">
      ...
      <WorkflowStepper active={confirmed ? 3 : 2} hrefs={["/refining-orders", `/refining-orders/${orderReference}/approval`, `/refining-orders/${orderReference}/dispatch`, undefined, undefined, `/refining-orders/${orderReference}/reserve-eligibility`]} labels={fr ? ["Brouillon", "Approuvé", "Expédition", "En raffinage", "Outturn reçu", "Classification"] : ["Draft", "Approved", "Dispatch", "In refining", "Outturn received", "Classification"]} />
      ...
      <Tabs defaultValue="details">
        ...
        <TabsContent value="details" className="mt-4 space-y-4">
          ...
          <RefiningPanel icon={FileText} title={fr ? "Contenu de l’envoi et contrôle du poids" : "Consignment contents & weight verification"}>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3"><InfoCell label={fr ? "Type d’or" : "Gold type"}>Doré bars</InfoCell><Field label={fr ? "Nombre de pièces" : "Number of pieces"} required><Input type="number" value={pieces} placeholder="42" disabled={confirmed} onChange={(event) => setPieces(event.target.value)} /></Field><InfoCell label={fr ? "Poids brut du lot" : "Lot record — gross"}>{order ? `${lotGross.toFixed(3)} kg` : "—"}</InfoCell><Field label={fr ? "Poids brut vérifié à l’expédition (kg)" : "Verified gross weight at dispatch (kg)"} required><Input type="number" step="0.001" value={dispatchWeight} placeholder={order ? order.inputGrossWeightKg.toFixed(3) : "50.000"} disabled={confirmed} onChange={(event) => setDispatchWeight(event.target.value)} /></Field><InfoCell label={fr ? "Essai / pureté" : "Assay / purity"}>{order ? `${purity.toFixed(2)} %` : "—"}</InfoCell><InfoCell label={fr ? "Teneur en or fin" : "Fine gold content"}>{order ? `${fineKg.toFixed(3)} kg · ${(fineKg * OZ_PER_KG).toLocaleString("en-US", { maximumFractionDigits: 2 })} oz` : "—"}</InfoCell></div>
          ...
          <RefiningPanel icon={LockKeyhole} title={fr ? "Sécurité et scellés inviolables" : "Security & tamper-evident seals"}>
            <div className="mb-4 grid gap-4 md:grid-cols-2"><Field label={fr ? "Type d’emballage" : "Packaging type"}><Select value={packaging} disabled={confirmed} onValueChange={setPackaging}><SelectTrigger><SelectValue placeholder={fr ? "Sélectionner un emballage" : "Select packaging"} /></SelectTrigger><SelectContent><SelectItem value="case">{fr ? "Caisse de sécurité scellée" : "Sealed security case"}</SelectItem><SelectItem value="pouch">{fr ? "Pochette inviolable" : "Tamper-evident pouch"}</SelectItem><SelectItem value="pallet">{fr ? "Palette cerclée" : "Strapped pallet"}</SelectItem></SelectContent></Select></Field><Field label={fr ? "Référence balance" : "Weighbridge / scale reference"}><Input value={scaleReference} placeholder="SCALE-KIN-VLT-02" disabled={confirmed} onChange={(event) => setScaleReference(event.target.value)} /></Field></div>
            <Label>{fr ? "Numéros de scellés" : "Seal numbers"} <span className="text-destructive">*</span></Label>
            <div className="mt-2 space-y-2">{seals.map((seal, index) => <div key={index} className="flex gap-2"><Input value={seal} disabled={confirmed} onChange={(event) => updateSeal(index, event.target.value)} placeholder={`SEAL-KIN-${88213 + index}`} /><Button type="button" size="icon" variant="ghost" disabled={confirmed || seals.length === 1} onClick={() => setSeals((previous) => previous.filter((_, current) => current !== index))}><X className="h-4 w-4" /></Button></div>)}</div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <RefiningPanel icon={Truck} title={fr ? "Transport et convoyeur" : "Transport & carrier"}><div className="space-y-4"><Field label={fr ? "Transporteur de sécurité" : "Security carrier"} required><Input value={carrier} placeholder="G4S Secure Transport" disabled={confirmed} onChange={(event) => setCarrier(event.target.value)} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label={fr ? "Référence convoi / véhicule" : "Convoy / vehicle ref"}><Input value={vehicle} placeholder="CONVOY-2026-0714" disabled={confirmed} onChange={(event) => setVehicle(event.target.value)} /></Field><Field label={fr ? "Mode" : "Mode"}><Select value={transportMode} disabled={confirmed} onValueChange={setTransportMode}><SelectTrigger><SelectValue placeholder={fr ? "Sélectionner un mode" : "Select mode"} /></SelectTrigger><SelectContent><SelectItem value="armoured">{fr ? "Convoi routier blindé" : "Armoured road convoy"}</SelectItem><SelectItem value="air">{fr ? "Fret aérien sécurisé" : "Secured air freight"}</SelectItem></SelectContent></Select></Field><Field label={fr ? "Date et heure d’expédition" : "Dispatch date & time"} required><Input type="datetime-local" value={dispatchAt} placeholder="23/07/2026 09:15" disabled={confirmed} onChange={(event) => setDispatchAt(event.target.value)} /></Field><Field label={fr ? "Arrivée prévue" : "Expected arrival"}><Input type="datetime-local" value={arrivalAt} placeholder="23/07/2026 11:00" disabled={confirmed} onChange={(event) => setArrivalAt(event.target.value)} /></Field></div></div></RefiningPanel>
            <RefiningPanel icon={Shield} title={fr ? "Assurance en transit" : "In-transit insurance"}><div className="grid gap-4 sm:grid-cols-2"><Field label={fr ? "Assureur" : "Insurer"} required><Input value={insurer} placeholder="SONAS / Lloyd's syndicate" disabled={confirmed} onChange={(event) => setInsurer(event.target.value)} /></Field><Field label={fr ? "Numéro de police" : "Policy number"}><Input value={policyNumber} placeholder="POL-AU-2026-3391" disabled={confirmed} onChange={(event) => setPolicyNumber(event.target.value)} /></Field><Field label={fr ? "Couverture" : "Coverage"}><Select value={coverage} disabled={confirmed} onValueChange={setCoverage}><SelectTrigger><SelectValue placeholder={fr ? "Sélectionner une couverture" : "Select coverage"} /></SelectTrigger><SelectContent><SelectItem value="all-risk">{fr ? "Tous risques, transit" : "All-risk, in-transit"}</SelectItem><SelectItem value="named">{fr ? "Risques désignés" : "Named-perils"}</SelectItem></SelectContent></Select></Field><Field label={fr ? "Valide jusqu’au" : "Valid through"}><Input type="date" value={validThrough} placeholder="31/08/2026" disabled={confirmed} onChange={(event) => setValidThrough(event.target.value)} /></Field></div><div className="mt-4 grid gap-4 border-t pt-4 sm:grid-cols-2"><InfoCell label={fr ? "Référence marché (LBMA)" : "Market reference (LBMA)"}>{order ? `${order.goldPricePerOz.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD/oz` : "—"}</InfoCell><InfoCell label={fr ? "Valeur assurée (or fin)" : "Insured value (fine gold)"}><span className="text-primary">{order ? `${Math.round(insuredValue).toLocaleString("en-US")} USD` : "—"}</span></InfoCell></div></RefiningPanel>
          </div>
