...
export function NonMonetaryHoldings() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-xl font-semibold">{fr ? "Sous-livre de détention non monétaire" : "Non-monetary holding sub-ledger"}</h1><p className="mt-1 text-xs text-muted-foreground">{fr ? "Or classé non monétaire à l’étape US-R05 · autres actifs en devises · suivi de la remédiation vers l’éligibilité aux réserves (US-R06)" : "Gold classified non-monetary at US-R05 · held as other foreign currency assets · tracked for remediation to reserve eligibility (US-R06)"}</p></div><Button variant="outline" size="sm" onClick={exportLedger}><Download className="mr-2 h-4 w-4" />{fr ? "Exporter le sous-livre" : "Export sub-ledger"}</Button></div>
...
function HoldingStatusPill({ holding, fr }: { holding: Holding; fr: boolean }) {
  ...
  if (holding.status === "remediated") return <StatusPill tone="success">{fr ? "Réévaluation US-R05" : "Re-evaluating US-R05"}</StatusPill>;
...
function Remediation({ holding, fr, onReRefine, onWatch, onReEvaluate }: { holding: Holding; fr: boolean; onReRefine: () => void; onWatch: () => void; onReEvaluate: () => void }) {
  return <div><p className="mb-3 text-sm font-semibold">{fr ? "Remédiation" : "Remediation"}</p>{holding.status === "held" && <div className="space-y-3"><div className="rounded-lg border p-4"><div className="flex items-center gap-2 text-sm font-semibold"><Factory className="h-4 w-4 text-violet-500" />{fr ? "Raffiner via une raffinerie accréditée" : "Re-refine via accredited refiner"}</div><p className="my-3 text-xs leading-5 text-muted-foreground">{fr ? "Créer un nouvel ordre US-R02 vers une raffinerie LBMA Good Delivery, par exemple Rand Refinery. L’outturn conforme reviendra à l’évaluation US-R05." : "Create a new US-R02 order routed to an LBMA Good Delivery refiner. Accepted GD-standard outturn re-enters US-R05."}</p><Button size="sm" onClick={onReRefine}>{fr ? "Créer l’ordre de nouveau raffinage" : "Create re-refining order"}</Button></div><div className="rounded-lg border p-4"><div className="flex items-center gap-2 text-sm font-semibold"><Clock3 className="h-4 w-4 text-sky-500" />{fr ? "Attendre l’accréditation" : "Await refiner accreditation"}</div><p className="my-3 text-xs leading-5 text-muted-foreground">{fr ? "Surveiller le statut LBMA GD de la raffinerie. Une accréditation déclenche une nouvelle évaluation US-R05." : "Watch the refiner’s LBMA GD status. Accreditation flags the holding for US-R05 re-evaluation."}</p><Button size="sm" variant="outline" onClick={onWatch}>{fr ? "Ajouter à la surveillance" : "Add to accreditation watch"}</Button></div>{holding.reason === "Sourcing verification pending" && <div className="rounded-lg border bg-muted/30 p-3 text-xs text-muted-foreground">{fr ? "Ce lot échoue sur l’approvisionnement : compléter la vérification OCDE / approvisionnement responsable avant réévaluation US-R05." : "This holding fails sourcing: complete OECD / responsible-sourcing verification before US-R05 re-evaluation."}</div>}</div>}
    ...
    {holding.status === "remediated" && <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-4 text-xs text-muted-foreground"><strong className="text-foreground">{fr ? "Transmis pour réévaluation." : "Sent for re-evaluation."}</strong> {fr ? "Le lot retourne à l’éligibilité aux réserves US-R05." : "The holding is routed back to US-R05."}</div>}
  </div>;
}
