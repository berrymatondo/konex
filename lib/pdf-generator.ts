...
export function buildBccSettlementPDF(record:{reference:string;purchaseOrder:string;data:Record<string,any>;createdAt:string},language:"fr"|"en"="fr"):ArrayBuffer{
  const fr=language==="fr",d=record.data||{},doc=new jsPDF({unit:"mm",format:"a4"}),width=doc.internal.pageSize.getWidth(),costs=Array.isArray(d.costs)?d.costs:[]
  doc.setFillColor(18,24,33);doc.rect(0,0,width,50,"F");doc.setFillColor(202,163,72);doc.rect(0,50,width,3,"F");doc.setTextColor(255,255,255);doc.setFont("helvetica","bold");doc.setFontSize(20);doc.text("BANQUE CENTRALE DU CONGO",20,20);doc.setFontSize(14);doc.text(fr?"CONFIRMATION DE REGLEMENT":"SETTLEMENT CONFIRMATION",20,33)
  let y=63;doc.setFillColor(241,245,249);doc.roundedRect(20,y,width-40,23,3,3,"F");doc.setTextColor(100,116,139);doc.setFontSize(8);doc.text(fr?"REFERENCE DU REGLEMENT":"SETTLEMENT REFERENCE",25,y+7);doc.setTextColor(30,41,59);doc.setFontSize(16);doc.text(record.reference,25,y+17);doc.setFillColor(34,197,94);doc.roundedRect(width-56,y+6,31,11,2,2,"F");doc.setTextColor(255,255,255);doc.setFontSize(8);doc.text(fr?"REGLE":"SETTLED",width-49,y+13)
  const value=(input:unknown,decimals=2)=>`${Number(input||0).toLocaleString(fr?"fr-FR":"en-GB",{minimumFractionDigits:decimals,maximumFractionDigits:decimals})}`
  const row=(label:string,text:string)=>{doc.setFont("helvetica","normal");doc.setFontSize(9);doc.setTextColor(100,116,139);doc.text(label,25,y);doc.setTextColor(30,41,59);doc.text(String(text||"—"),92,y);y+=8}
  const section=(title:string)=>{y+=15;doc.setTextColor(30,41,59);doc.setFont("helvetica","bold");doc.setFontSize(11);doc.text(title,20,y);y+=5;doc.setDrawColor(226,232,240);doc.line(20,y,width-20,y);y+=9}
  y=92;section(fr?"ORDRE D'ACHAT LIE":"LINKED PURCHASE ORDER");row(fr?"Numero du PO":"PO number",record.purchaseOrder);row(fr?"Date du document":"Document date",new Date(record.createdAt).toLocaleDateString(fr?"fr-FR":"en-GB"));row(fr?"Reference de confirmation":"Confirmation reference",d.settlementConfirmationReference||"—")
  section(fr?"TARIFICATION ET REGLEMENT":"PRICING AND SETTLEMENT");row(fr?"Prix execute":"Executed price",`${value(d.executedPrice)} USD/oz`);row(fr?"Prime / decote":"Premium / discount",`${value(d.settlementPremium)} USD`);row(fr?"Change date de transaction":"Trade-date FX",`${value(d.tradeFx)} CDF/USD`);row(fr?"Change date de paiement":"Payment-date FX",`${value(d.paymentFx)} CDF/USD`);row(fr?"Effet de change":"Settlement FX effect",`${value(d.effectiveSettlementFxEffect)} USD`);row(fr?"Contrepartie finale":"Final consideration",`${value(d.finalPurchaseConsideration)} USD`)
  const totals=costs.reduce((sum:{capitalized:number;expense:number;memo:number;total:number},cost:any)=>{const amount=Number(cost.usd||0);sum.total+=amount;if(cost.treatment==="capitalized")sum.capitalized+=amount;else if(cost.treatment==="expense")sum.expense+=amount;else sum.memo+=amount;return sum},{capitalized:0,expense:0,memo:0,total:0})
  section(fr?"COUTS ENREGISTRES":"RECORDED COSTS");row(fr?"Couts capitalises":"Capitalized costs",`${value(totals.capitalized)} USD`);row(fr?"Charges de la periode":"Period expenses",`${value(totals.expense)} USD`);row(fr?"Memo / exclus":"Memo / excluded",`${value(totals.memo)} USD`);row(fr?"Total enregistre":"Total recorded",`${value(totals.total)} USD`)
  const footer=doc.internal.pageSize.getHeight()-18;doc.setDrawColor(226,232,240);doc.line(20,footer-5,width-20,footer-5);doc.setTextColor(100,116,139);doc.setFontSize(8);doc.text(fr?"Document genere automatiquement - Confidentiel":"Automatically generated document - Confidential",20,footer)
  return doc.output("arraybuffer")
}

export function buildBccCustodyPDF(record:{reference:string;purchaseOrder:string;data:Record<string,any>},language:"fr"|"en"="fr"):ArrayBuffer{const fr=language==="fr",d=record.data||{},doc=new jsPDF({unit:"mm",format:"a4"}),w=doc.internal.pageSize.getWidth();doc.setFillColor(18,24,33);doc.rect(0,0,w,50,"F");doc.setFillColor(202,163,72);doc.rect(0,50,w,3,"F");doc.setTextColor(255,255,255);doc.setFont("helvetica","bold");doc.setFontSize(20);doc.text("BANQUE CENTRALE DU CONGO",20,20);doc.setFontSize(14);doc.text(fr?"CONFIRMATION DE CONSERVATION":"CUSTODY CONFIRMATION",20,33);let y=68;const row=(l:string,v:unknown)=>{doc.setFontSize(9);doc.setTextColor(100,116,139);doc.text(l,25,y);doc.setTextColor(30,41,59);doc.text(String(v||"—"),92,y);y+=10};row(fr?"Reference":"Reference",record.reference);row(fr?"Ordre d'achat":"Purchase order",record.purchaseOrder);row(fr?"Depositaire":"Depositary",d.depositary);row(fr?"Coffre / emplacement":"Vault / location",d.vaultLocation);row(fr?"Compte de conservation":"Custody account",d.custodyAccount);row(fr?"Modele de conservation":"Custody model",d.custodyModel);row(fr?"Recepisse":"Warehouse receipt",d.warehouseReceipt);row(fr?"Police d'assurance":"Insurance policy",d.insurancePolicy);row(fr?"Titre libre de charge":"Unencumbered title",d.unencumbered?(fr?"Confirme":"Confirmed"):(fr?"Non confirme":"Not confirmed"));return doc.output("arraybuffer")}
