...
const DOC_TYPE_LABELS: Record<string, { fr: string; en: string }> = {
  ...
  carrier_waybill: { fr: "Lettre de voiture transporteur", en: "Carrier Waybill" },
...
export default function PurchaseOrderDetailPage() {
  ...
  const { data: po, mutate } = useSWR<PurchaseOrder>(`/api/purchase-orders/${id}`, fetcher);
  ...
  const [failedDocTypesSelected, setFailedDocTypesSelected] = useState<string[]>([]);
  const [publicNotes, setPublicNotes] = useState("");
  const [internalNotes, setInternalNotes] = useState("");
  const [reviewSubmitting, setReviewSubmitting] = useState<"validate" | "return" | null>(null);
  const [reviewResult, setReviewResult] = useState<{ type: "success" | "error"; text: string } | null>(null);

  const MANIFEST_DOC_LABELS: Record<string, string> = {
    export_permit: language === "fr" ? "Permis d'exportation" : "Export Permit",
    assay_certificate: language === "fr" ? "Certificats d'analyse" : "Assay Certificates",
    chain_of_custody: language === "fr" ? "Chaîne de garde" : "Chain of Custody",
    carrier_waybill: language === "fr" ? "Lettre de voiture" : "Carrier Waybill",
    lbma_rgg: "LBMA RGG",
    minamata: language === "fr" ? "Déclaration Minamata" : "Minamata Declaration",
  };

  const handleManifestReview = async (action: "validate" | "return") => {
    if (action === "return" && !reasonCode && !publicNotes.trim()) {
      setReviewResult({ type: "error", text: language === "fr" ? "Sélectionnez un motif ou saisissez une note publique." : "Select a reason code or enter a public note." });
      return;
    }
    ...
    try {
      ...
      setReviewResult({
        ...
        text: action === "validate"
          ? (language === "fr" ? "Manifeste validé avec succès." : "Manifest validated successfully.")
          : (language === "fr"
      ...
      setReviewResult({ type: "error", text: e instanceof Error ? e.message : "Erreur" });
    } finally {
  ...
  const [approvals, setApprovals] = useState<{ role: string; name: string; timestamp: Date; otp?: string }[]>([]);
  const [currentApproverOTP, setCurrentApproverOTP] = useState("");
  const [otpVerified, setOtpVerified] = useState(false);
  const [approvalNotes, setApprovalNotes] = useState("");

  // Dispatch tracking states
  const [dispatchStatus, setDispatchStatus] = useState<"awaiting" | "in_transit" | "received">("awaiting");
  ...
  const runComplianceRecheck = async () => {
    ...
    setComplianceChecks(prev => ({ ...prev, counterpartyStatus: counterparty?.status === "active" ? "passed" : "failed" }));
    
    await new Promise(r => setTimeout(r, 500));
    const eddRequired = counterparty?.riskLevel === "high";
    setComplianceChecks(prev => ({ 
      ...prev, 
      eddComplete: eddRequired ? (counterparty?.screeningStatus === "passed" ? "passed" : "failed") : "na" 
    }));
  ...
  const handleVerifyOTP = () => {
    if (!currentApproverOTP || currentApproverOTP.length !== 6) {
      alert(language === "fr" ? "Veuillez entrer un OTP valide à 6 chiffres" : "Please enter a valid 6-digit OTP");
      return;
    }
  ...
  const handleApprove = async () => {
    ...
    if (!otpVerified) {
      alert(language === "fr" ? "Veuillez d'abord vérifier votre code OTP" : "Please verify your OTP code first");
      return;
    }
 ...
 const handleReject = async () => {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSendResult({
          type: "success",
          text: language === "fr" ? `PDF envoyé à ${data.email}` : `PDF sent to ${data.email}`,
        });
      } else {
        setSendResult({
          type: "error",
      ...
      setSendResult({
        type: "error",
        text: language === "fr" ? "Échec de l'envoi de l'email." : "Failed to send email.",
      });
    } finally {
  ...
  const handleSubmitToCounterparty = async () => {
    ...
    setSendResult(null);
    try {
      const res = await fetch(`/api/purchase-orders/${id}/submit-to-counterparty`, { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (res.ok) {
        setSendResult({
          type: "success",
        ...
        setSendResult({
          type: "error",
          text:
            (language === "fr" ? "Échec de la soumission : " : "Submission failed: ") +
            (data.error || res.statusText),
        });
      }
    } catch {
      setSendResult({
        type: "error",
        text: language === "fr" ? "Échec de la soumission à la contrepartie." : "Failed to submit to counterparty.",
      });
    } finally {
  ...
  const handleCancelTransmittedOrder = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSendResult({
          type: "success",
          text: language === "fr" ? "Bon de commande annulé." : "Purchase order cancelled.",
        });
        mutate();
      } else {
        setSendResult({
          type: "error",
          text:
            (language === "fr" ? "Échec de l'annulation : " : "Cancellation failed: ") +
            (data.error || res.statusText),
        });
      }
    } catch {
      setSendResult({
        type: "error",
        text: language === "fr" ? "Échec de l'annulation du bon de commande." : "Failed to cancel purchase order.",
      });
    } finally {
  ...
  const statusConfig: Record<string, { label: string; className: string; icon: typeof Clock }> = {
    draft: { label: language === "fr" ? "Brouillon" : "Draft", className: "border-muted-foreground text-muted-foreground", icon: FileText },
    submitted: { label: language === "fr" ? "Soumis" : "Submitted", className: "border-warning text-warning", icon: Clock },
    approved: { label: language === "fr" ? "Approuvé" : "Approved", className: "border-success text-success", icon: CheckCircle2 },
    rejected: { label: language === "fr" ? "Rejeté" : "Rejected", className: "border-destructive text-destructive", icon: XCircle },
    in_transit: { label: language === "fr" ? "En Transit" : "In Transit", className: "border-info text-info", icon: Truck },
    received: { label: language === "fr" ? "Reçu" : "Received", className: "border-success text-success", icon: CheckCircle2 },
    sent_to_counterparty: { label: language === "fr" ? "Transmis à la contrepartie" : "Sent to Counterparty", className: "border-info text-info", icon: Send },
    accepted: { label: language === "fr" ? "Accepté" : "Accepted", className: "border-success text-success", icon: CheckCircle2 },
  ...
  const canRespondToOffer = po.status === "sent_to_counterparty";
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto p-4 sm:p-6">
            <div className="mx-auto max-w-6xl space-y-4 sm:space-y-6">
              ...
              {searchParams.get("manifest") === "submitted" && (
                <Alert className="border-emerald-500/50 bg-emerald-500/10">
                  ...
                  <AlertTitle className="text-emerald-700">
                    {language === "fr" ? "Manifeste soumis avec succès" : "Manifest submitted successfully"}
                  </AlertTitle>
              ...
              {justSubmitted && (
                <Alert className="border-success bg-success/10">
                  ...
                  <AlertTitle className="text-success">
                    {language === "fr" ? "Bon de Commande Soumis" : "Purchase Order Submitted"}
                  </AlertTitle>
                  <AlertDescription>
                    {language === "fr" 
                    ...
                    <div className="flex-1">
                      ...
                      {po.tracking_id && (
                        <div className="mt-4 flex items-center gap-3">
                          <Badge variant="outline" className="border-emerald-400 text-emerald-300 text-lg px-4 py-1">
                            <QrCode className="mr-2 h-4 w-4" />
                            {po.tracking_id}
                          </Badge>
                          <Button variant="ghost" size="sm" className="text-emerald-300 hover:text-emerald-200" onClick={() => navigator.clipboard.writeText(po.tracking_id || "")}>
              ...
              {po.status === "sent_to_counterparty" && (
                <Alert className="border-info bg-info/10">
                  ...
                  <AlertTitle className="text-info">
                    {language === "fr" ? "Transmis à la contrepartie" : "Sent to Counterparty"}
                  </AlertTitle>
                  <AlertDescription>
                    {language === "fr"
              ...
              {(po.status === "accepted" || po.status === "negotiating" || po.status === "declined") && (
                <Alert
                  className={
                    po.status === "accepted"
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                    ...
                    {requiresDualApproval && (
                      <Badge variant="secondary" className="hidden sm:inline-flex">
                        ...
                        {language === "fr" ? "Double Approbation" : "Dual Approval"}
                      </Badge>
                    )}
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">
                    {language === "fr" ? "Créé le" : "Created"} {new Date(po.created_at).toLocaleDateString()}
                  </p>
                </div>
                  ...
                  {!isCounterparty && po.status === "negotiating" && (
                    <Link href={`/purchase-orders/${id}/edit`} className="flex-1 sm:flex-none">
                      <Button size="sm" className="w-full">
                        ...
                        <span>{language === "fr" ? "Modifier et re-soumettre" : "Modify & Resubmit"}</span>
                      </Button>
                    </Link>
                  )}
                </div>
              ...
              <Tabs defaultValue={hasSubmittedManifest && !isCounterparty ? "manifest" : po.status === "approved" && !isCounterparty ? "tracking" : "details"} className="space-y-6">
                <TabsList className="w-full sm:w-auto grid grid-cols-2 sm:inline-flex">
                  ...
                  {(po.status === "approved" || po.status === "in_transit") && !isCounterparty && (
                  ...
                  {hasSubmittedManifest && !isCounterparty && (
                    <TabsTrigger value="manifest" className="text-xs sm:text-sm">
                ...
                <TabsContent value="details" className="space-y-4 sm:space-y-6">
                  {(() => {
                    ...
                    const goldTypeLabel =
                      GOLD_TYPE_LABELS[po.gold_type]?.[language === "fr" ? "fr" : "en"] ??
                      po.gold_type?.replace(/_/g, " ");
                    const vaultLabel =
                      VAULT_LABELS[po.delivery_vault_id]?.[language === "fr" ? "fr" : "en"] ?? po.delivery_vault_id;
                      ...
                      <div className="grid gap-4 sm:gap-6 lg:grid-cols-3">
                        ...
                        <div className="lg:col-span-2 space-y-4 sm:space-y-6">
                            ...
                            <CardContent>
                              <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
                                ...
                                <RefField
                                  ...
                                  value={new Date(po.created_at).toLocaleDateString(locale)}
                                />
                          ...
                          <Card>
                            ...
                            <CardContent className="space-y-4">
                              ...
                              <div className="rounded-lg border bg-muted/40 p-4">
                                <p className="font-medium">{counterparty?.legalName || po.counterparty_name || "—"}</p>
                                <p className="text-sm text-muted-foreground capitalize">{counterparty?.status || "—"}</p>
                              </div>
                            </CardContent>
                          </Card>
                          ...
                          <Card>
                            ...
                            <CardContent>
                              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                                <RefField
                                  label={language === "fr" ? "Quantité cible" : "Target Quantity"}
                                  value={`${num(weightKg, 3)} kg`}
                                />
                                <RefField
                                  label={language === "fr" ? "Type d'Or" : "Gold Type"}
                                  value={goldTypeLabel}
                                />
                                <RefField
                                  label={language === "fr" ? "Plage de pureté" : "Assay Range"}
                                  value={po.assay_range ? `${po.assay_range} %` : "—"}
                                />
                                <RefField label="Incoterm" value={po.incoterms || "—"} />
                                <RefField
                                  label={language === "fr" ? "Pureté centrale" : "Central Purity"}
                                  value={`${num(purityFactor * 100, 2)} %`}
                                />
                                <RefField
                                  label={language === "fr" ? "Poids d'or fin estimé" : "Estimated Fine Gold"}
                          ...
                          <Card>
                            ...
                            <CardContent className="space-y-4">
                              <div className="grid grid-cols-2 gap-4">
                                <RefField
                                  label={language === "fr" ? "Coffre de destination" : "Destination Vault"}
                                  value={vaultLabel || "—"}
                                />
                                <RefField
                                  label={language === "fr" ? "Fenêtre de livraison souhaitée" : "Desired Delivery Window"}
                                  value={
                              ...
                              <div className="space-y-1">
                                <p className="text-xs text-muted-foreground">{language === "fr" ? "Notes" : "Notes"}</p>
                                <p className="text-sm">
                                  {po.notes || (language === "fr" ? "Aucune note" : "No notes")}
                                </p>
                              </div>
                            </CardContent>
                          </Card>
                          ...
                          <Card>
                            ...
                            <CardContent>
                              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                                <RefField label={language === "fr" ? "Devise" : "Currency"} value={po.currency || "—"} />
                                <RefField
                                  label={language === "fr" ? "Répartition USD / CDF" : "USD / CDF Split"}
                                  value={po.payment_usd_cdf_split || "—"}
                                />
                                <RefField
                                  label={language === "fr" ? "Moment du paiement" : "Payment Timing"}
                                  value={
                                    po.payment_timing === "after_delivery"
                                      ? language === "fr" ? "Après livraison" : "After delivery"
                                      : po.payment_timing === "on_delivery"
                                        ? language === "fr" ? "À la livraison" : "On delivery"
                                        : po.payment_timing === "before_delivery"
                                          ? language === "fr" ? "Avant livraison" : "Before delivery"
                                          : po.payment_timing || "—"
                                  }
                                />
                                <RefField
                                  label={language === "fr" ? "Délai de paiement" : "Payment Term"}
                                  value={po.payment_term || "—"}
                                />
                                <RefField
                                  label={language === "fr" ? "Prépaiement" : "Prepayment"}
                                  value={po.prepayment_percent != null ? `${num(Number(po.prepayment_percent), 0)} %` : "—"}
                                />
                                <RefField
                                  label={language === "fr" ? "Taux de change part CDF" : "CDF FX Basis"}
                                  value={
                        ...
                        <div className="space-y-6">
                          <Card className="sticky top-6">
                            <CardHeader className="pb-3">
                              ...
                              <CardDescription>
                                {language === "fr" ? "Références de marché et valorisation" : "Market references and valuation"}
                              </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                              <div className="flex items-center justify-between rounded-lg border-2 border-emerald-500 bg-emerald-500/10 p-3">
                                ...
                                <span className="font-mono text-sm font-bold text-emerald-600 dark:text-emerald-400">
                                  {num(Number(po.lbma_price_per_oz || 0), 2)} USD/oz
                                </span>
                              </div>
                              ...
                              <div className="space-y-2 text-sm">
                                ...
                                />
                                <MetricRow
                                  label={language === "fr" ? "Prime / remise" : "Premium / Discount"}
                                  value={num(Number(po.premium_discount || 0), 2)}
                                />
                                <MetricRow
                                  label={language === "fr" ? "Coût logistique" : "Logistics Cost"}
                              ...
                              <div className="flex flex-col items-center justify-center rounded-lg bg-primary/10 p-4 text-center">
                                <span className="text-sm text-muted-foreground">{language === "fr" ? "Montant total estimé" : "Total Estimated Amount"}</span>
                                <span className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                                  {num(Number(po.total_estimated_value || 0), 0)} {po.currency || "USD"}
                                </span>
                              </div>
                            </CardContent>
                          </Card>
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle className="flex items-center gap-2">
                            ...
                            {language === "fr" ? "Vérification de Conformité" : "Compliance Validation"}
                          </CardTitle>
                          <CardDescription>
                            {language === "fr" 
                        ...
                        <CardContent className="space-y-4">
                          ...
                          <div className="flex items-center justify-between p-3 rounded-lg border">
                            <div className="flex items-center gap-3">
                              {complianceChecks.counterpartyStatus === "pending" && <Clock className="h-5 w-5 text-muted-foreground animate-pulse" />}
                              {complianceChecks.counterpartyStatus === "passed" && <CheckCircle2 className="h-5 w-5 text-success" />}
                              {complianceChecks.counterpartyStatus === "failed" && <XCircle className="h-5 w-5 text-destructive" />}
                              <span>{language === "fr" ? "Statut Contrepartie: APPROUVÉ" : "Counterparty Status: APPROVED"}</span>
                            </div>
                            {complianceChecks.counterpartyStatus === "passed" && (
                              <Badge variant="outline" className="border-success text-success">✓</Badge>
                            )}
                          </div>
                          ...
                          <div className="flex items-center justify-between p-3 rounded-lg border">
                            <div className="flex items-center gap-3">
                              {complianceChecks.eddComplete === "pending" && <Clock className="h-5 w-5 text-muted-foreground animate-pulse" />}
                              {complianceChecks.eddComplete === "passed" && <CheckCircle2 className="h-5 w-5 text-success" />}
                              {complianceChecks.eddComplete === "na" && <CheckCircle2 className="h-5 w-5 text-muted-foreground" />}
                              {complianceChecks.eddComplete === "failed" && <XCircle className="h-5 w-5 text-destructive" />}
                              <span>{language === "fr" ? "EDD Complet (si HIGH risk)" : "EDD Complete (if HIGH risk)"}</span>
                            </div>
                          ...
                          <div className="flex items-center justify-between p-3 rounded-lg border">
                            <div className="flex items-center gap-3">
                              {complianceChecks.sanctionsRecheck === "pending" && <Clock className="h-5 w-5 text-muted-foreground animate-pulse" />}
                              {complianceChecks.sanctionsRecheck === "passed" && <CheckCircle2 className="h-5 w-5 text-success" />}
                              {complianceChecks.sanctionsRecheck === "failed" && <XCircle className="h-5 w-5 text-destructive" />}
                              <span>{language === "fr" ? "Re-vérification Sanctions: CLEAR" : "Sanctions Re-check: CLEAR"}</span>
                            </div>
                            {complianceChecks.sanctionsRecheck === "passed" && (
                              <Badge variant="outline" className="border-success text-success">✓</Badge>
                            )}
                          ...
                          >
                            <RefreshCw className={`mr-2 h-4 w-4 ${isRecheckingCompliance ? "animate-spin" : ""}`} />
                            {language === "fr" ? "Relancer la vérification" : "Re-run Compliance Check"}
                          </Button>
                        </CardContent>
                      </Card>
                          ...
                          <CardTitle className="flex items-center gap-2">
                            ...
                            {requiresDualApproval && (
                              <Badge variant="secondary" className="ml-2">
                                {language === "fr" ? "Double requis" : "Dual Required"}
                              </Badge>
                            )}
                          </CardTitle>
                          <CardDescription>
                            {requiresDualApproval 
                              ...
                              : (language === "fr" ? "Une approbation requise" : "One approval required")}
                          </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                          {/* Progress */}
                          <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                              ...
                              <span>{approvals.length}/{approvalsNeeded}</span>
                            </div>
                            <Progress value={(approvals.length / approvalsNeeded) * 100} />
                          </div>

                          {/* Existing Approvals */}
                          {approvals.map((approval, idx) => (
                            <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-success/10 border border-success/30">
                              <div className="flex items-center gap-3">
                                <CheckCircle2 className="h-5 w-5 text-success" />
                                <div>
                          ...
                          {!approvalsComplete && allCompliancesPassed && (
                            <div className="space-y-3 pt-4 border-t">
                              <Label>{language === "fr" ? "Code OTP Approbateur" : "Approver OTP Code"}</Label>
                              <div className="flex gap-2">
                                <Input
                                  type="text"
                                  maxLength={6}
                                  ...
                                  onChange={(e) => {
                                    setCurrentApproverOTP(e.target.value.replace(/\D/g, ""));
                                ...
                                {!otpVerified ? (
                                  ...
                                  <Button variant="outline" className="border-success text-success" disabled>
                          ...
                          {!allCompliancesPassed && (
                            <Alert variant="destructive">
                              ...
                              <AlertTitle>{language === "fr" ? "Vérification requise" : "Verification Required"}</AlertTitle>
                              <AlertDescription>
                                {language === "fr" 
                          ...
                          {!poValidation.isValid && (
                            <Alert variant="destructive" className="border-amber-500/50 bg-amber-500/10">
                              ...
                              <AlertTitle className="text-amber-600">
                                {language === "fr" ? "Informations incomplètes" : "Incomplete Information"}
                              </AlertTitle>
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="approval-notes">
                              {language === "fr" ? "Notes (obligatoire pour le rejet)" : "Notes (required for rejection)"}
                            </Label>
                            <Textarea
                              id="approval-notes"
                              value={approvalNotes}
  ...
  >
                            <CheckCircle2 className="mr-2 h-4 w-4" />
                            {language === "fr" ? "Approuver & Soumettre" : "Approve & Submit"}
                          </Button>
                        </CardFooter>
                      </Card>
                    </div>
                    ...
                    <Card>
                      <CardHeader>
                        <CardTitle>{language === "fr" ? "Historique d'Approbation" : "Approval History"}</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="space-y-3">
                          <div className="flex items-center gap-3 text-sm">
                            ...
                            <span>{language === "fr" ? "PO Créé" : "PO Created"}</span>
                          </div>
                          {po.submitted_at && (
                            <div className="flex items-center gap-3 text-sm">
                              <Send className="h-4 w-4 text-warning" />
                              <span>{new Date(po.submitted_at).toLocaleString()}</span>
                              <span className="text-muted-foreground">-</span>
                              <span>{language === "fr" ? "Soumis pour approbation" : "Submitted for approval"}</span>
                            </div>
                          )}
                          {approvals.map((a, idx) => (
                            <div key={idx} className="flex items-center gap-3 text-sm">
                              ...
                              <span className="text-muted-foreground">-</span>
                              <span>{a.name} ({a.role}) {language === "fr" ? "a approuvé" : "approved"}</span>
                            </div>
                          ))}
                        </div>
                      </CardContent>
                ...
                {(po.status === "approved" || po.status === "in_transit") && !isCounterparty && (
                  <TabsContent value="tracking" className="space-y-6">
                    ...
                    <Card>
                      <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                          ...
                          {language === "fr" ? "Suivi de l'Expédition" : "Dispatch Tracking"}
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex flex-col items-center">
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                              dispatchStatus === "awaiting" ? "bg-warning text-warning-foreground" : "bg-success text-success-foreground"
                            }`}>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {language === "fr" ? "d'expédition" : "Dispatch"}
                            </p>
                          </div>

                          {/* Connector */}
                          <div className={`flex-1 h-1 mx-4 ${dispatchStatus !== "awaiting" ? "bg-success" : "bg-muted"}`} />

                          {/* Stage 2 */}
                          <div className="flex flex-col items-center">
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                              dispatchStatus === "in_transit" ? "bg-info text-info-foreground" : 
                              dispatchStatus === "received" ? "bg-success text-success-foreground" : "bg-muted text-muted-foreground"
                            }`}>
                          ...
                          {/* Connector */}
                          <div className={`flex-1 h-1 mx-4 ${dispatchStatus === "received" ? "bg-success" : "bg-muted"}`} />
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                      <Card>
                        <CardHeader>
                          <CardTitle>{language === "fr" ? "Informations de Suivi" : "Tracking Information"}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                          <div className="flex items-center gap-3 p-4 rounded-lg bg-muted">
                            ...
                            <div>
                              ...
                              <p className="font-mono text-lg font-bold">{po.tracking_id || "GAC-TRK-PENDING"}</p>
                            </div>
                          </div>

                          <div className="space-y-2">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{language === "fr" ? "Coffre destination" : "Destination Vault"}</span>
                              <span className="font-medium">{po.delivery_vault_id}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{language === "fr" ? "Date prévue" : "Expected Date"}</span>
                              <span className="font-medium">{po.expected_dispatch_date || "-"}</span>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle>{language === "fr" ? "Prochaines Étapes" : "Next Steps"}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                          {dispatchStatus === "awaiting" && (
                            <>
                              <Alert>
                                ...
                                <AlertTitle>{language === "fr" ? "En attente de confirmation" : "Awaiting Confirmation"}</AlertTitle>
                                <AlertDescription>
                                  {language === "fr" 
                              ...
                              >
                                ...
                                {language === "fr" ? "Simuler Confirmation d'Expédition" : "Simulate Dispatch Confirmation"}
                              </Button>
                          ...
                          {dispatchStatus === "in_transit" && (
                            <Alert className="border-info">
                              ...
                              <AlertTitle>{language === "fr" ? "En Transit" : "In Transit"}</AlertTitle>
                              <AlertDescription>
                                {language === "fr" 
                          ...
                          {dispatchStatus === "received" && (
                            <Alert className="border-success">
                              <CheckCircle2 className="h-4 w-4 text-success" />
                              <AlertTitle>{language === "fr" ? "Reçu au Coffre" : "Received at Vault"}</AlertTitle>
                              <AlertDescription>
                                {language === "fr" 
                  ...
                  <TabsContent value="manifest" className="space-y-6">
                    ...
                    {manifest.status === "accepted" && (
                      <Alert className="border-emerald-500/50 bg-emerald-500/10">
                        ...
                        <AlertTitle className="text-emerald-700">
                          {language === "fr" ? "Manifeste validé" : "Manifest validated"}
                        </AlertTitle>
                        <AlertDescription className="text-emerald-700/80">
                          {language === "fr"
                    ...
                    {manifest.status === "returned" && (
                      <Alert className="border-amber-500/50 bg-amber-500/10">
                        ...
                        <AlertTitle className="text-amber-700">
                          {language === "fr" ? "Manifeste retourné à la contrepartie" : "Manifest returned to counterparty"}
                        </AlertTitle>
                    ...
                    <div className="grid gap-6 lg:grid-cols-3">
                      ...
                      <div className="lg:col-span-2 space-y-6">
                        ...
                        <Card>
                          <CardHeader className="pb-3">
                            <CardTitle className="flex items-center gap-2 text-base">
                              <Truck className="h-5 w-5 text-muted-foreground" />
                              {language === "fr" ? "Détails de l'expédition" : "Shipment Details"}
                            </CardTitle>
                            <CardDescription>
                              {language === "fr" ? "Tentative" : "Attempt"} #{manifest.attempt_number}
                              {manifest.submitted_at && ` — ${new Date(manifest.submitted_at).toLocaleString(language === "fr" ? "fr-FR" : "en-US")}`}
                            </CardDescription>
                          </CardHeader>
                          <CardContent>
                            <div className="grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-3">
                              {[
                                [language === "fr" ? "Date d'expédition" : "Shipment Date", manifest.shipment_date ? new Date(manifest.shipment_date).toLocaleDateString(language === "fr" ? "fr-FR" : "en-US") : "—"],
                                [language === "fr" ? "Transporteur" : "Carrier", manifest.carrier || "—"],
                                [language === "fr" ? "N° de lettre de voiture" : "Waybill No.", manifest.waybill_number || "—"],
                                [language === "fr" ? "Lieu de départ" : "Departure Location", manifest.departure_location || "—"],
                                [language === "fr" ? "Coffre de destination" : "Destination Vault", manifest.destination_vault || "—"],
                                ["Incoterms", manifest.incoterms || "—"],
                                [language === "fr" ? "N° de scellé" : "Seal Number", manifest.seal_number || "—"],
                              ].map(([label, value]) => (
                        ...
                        <Card>
                          <CardHeader className="pb-3">
                            <CardTitle className="flex items-center gap-2 text-base">
                              <Scale className="h-5 w-5 text-muted-foreground" />
                              {language === "fr" ? "Récapitulatif des poids" : "Weight Summary"}
                            </CardTitle>
                          </CardHeader>
                          <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                              <div className="rounded-lg bg-muted p-3 text-center">
                                <p className="text-2xl font-bold font-mono">{manifest.total_bars ?? "—"}</p>
                                <p className="text-xs text-muted-foreground mt-1">{language === "fr" ? "Lingots" : "Bars"}</p>
                              </div>
                              <div className="rounded-lg bg-muted p-3 text-center">
                                <p className="text-2xl font-bold font-mono">{manifest.total_gross_weight_kg != null ? Number(manifest.total_gross_weight_kg).toFixed(3) : "—"}</p>
                                <p className="text-xs text-muted-foreground mt-1">{language === "fr" ? "Poids brut (kg)" : "Gross Weight (kg)"}</p>
                              </div>
                              <div className="rounded-lg bg-muted p-3 text-center">
                                <p className="text-2xl font-bold font-mono">{manifest.total_fine_oz != null ? Number(manifest.total_fine_oz).toFixed(3) : "—"}</p>
                                <p className="text-xs text-muted-foreground mt-1">{language === "fr" ? "Poids fin (oz) manifeste" : "Fine Oz (manifest)"}</p>
                              </div>
                              <div className={`rounded-lg p-3 text-center ${
                                manifest.variance_percent != null && Math.abs(Number(manifest.variance_percent)) <= 0.5
                                ...
                                <p className="text-xs text-muted-foreground mt-1">{language === "fr" ? "Variance vs BCC" : "Variance vs PO"}</p>
                              </div>
                            </div>

                            {/* Bar list */}
                            {Array.isArray(manifest.bars_json) && (manifest.bars_json as unknown[]).length > 0 && (
                              <div className="overflow-x-auto rounded-lg border">
                                <table className="w-full text-sm">
                                  <thead>
                                    <tr className="border-b bg-muted/50">
                                      <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">#</th>
                                      <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">{language === "fr" ? "N° lingot" : "Bar No."}</th>
                                      <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">{language === "fr" ? "Poids brut (kg)" : "Gross (kg)"}</th>
                                      <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">{language === "fr" ? "Finesse" : "Fineness"}</th>
                                      <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">{language === "fr" ? "Poids fin (oz)" : "Fine Oz"}</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    {(manifest.bars_json as Array<{ barNumber?: string; grossWeightKg?: number; fineness?: number; fineOz?: number }>).map((bar, i) => (
                                      <tr key={i} className="border-b last:border-0 hover:bg-muted/30">
                                        <td className="px-3 py-2 text-muted-foreground">{i + 1}</td>
                                        <td className="px-3 py-2 font-mono">{bar.barNumber || "—"}</td>
                                        <td className="px-3 py-2 text-right font-mono">{bar.grossWeightKg != null ? Number(bar.grossWeightKg).toFixed(3) : "—"}</td>
                                        <td className="px-3 py-2 text-right font-mono">{bar.fineness != null ? Number(bar.fineness).toFixed(4) : "—"}</td>
                                        <td className="px-3 py-2 text-right font-mono">{bar.fineOz != null ? Number(bar.fineOz).toFixed(3) : "—"}</td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
                          ...
                          <CardContent>
                            <div className="space-y-2">
                              {Object.keys(DOC_TYPE_LABELS).map((docType) => {
                                const doc = manifest.documents?.find((d) => d.doc_type === docType);
                                const label = DOC_TYPE_LABELS[docType]?.[language === "fr" ? "fr" : "en"] ?? docType;
                                return (
                        ...
                        {/* Declaration */}
                        {(manifest.declarant_name || manifest.declaration_accepted_at) && (
                          <Card>
                            <CardHeader className="pb-3">
                              <CardTitle className="flex items-center gap-2 text-base">
                                <ShieldCheck className="h-5 w-5 text-muted-foreground" />
                                {language === "fr" ? "Déclaration du déclarant" : "Declarant's Declaration"}
                              </CardTitle>
                            </CardHeader>
                            <CardContent>
                              <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                  ...
                                  <p className="text-sm font-medium">{manifest.declarant_name || "—"}</p>
                                </div>
                                <div className="space-y-1">
                                  <p className="text-xs text-muted-foreground">{language === "fr" ? "Titre" : "Title"}</p>
                                  <p className="text-sm font-medium">{manifest.declarant_title || "—"}</p>
                                </div>
                                {manifest.declaration_accepted_at && (
                                  <div className="col-span-2 space-y-1">
                                    <p className="text-xs text-muted-foreground">{language === "fr" ? "Acceptée le" : "Accepted on"}</p>
                                    <p className="text-sm font-medium">{new Date(manifest.declaration_accepted_at).toLocaleString(language === "fr" ? "fr-FR" : "en-US")}</p>
                                  </div>
                                )}
                              </div>
                            </CardContent>
                      ...
                      <div className="space-y-4">
                            ...
                            <CardContent className="space-y-4">
                              ...
                              <div className="space-y-2">
                                <Label className="text-sm">
                                  {language === "fr" ? "Code motif (si retour)" : "Reason code (if returning)"}
                                </Label>
                                <Select value={reasonCode} onValueChange={setReasonCode}>
                                  ...
                                  <SelectContent>
                                    <SelectItem value="missing_document">{language === "fr" ? "Document manquant" : "Missing Document"}</SelectItem>
                                    <SelectItem value="weight_discrepancy">{language === "fr" ? "Écart de poids" : "Weight Discrepancy"}</SelectItem>
                                    <SelectItem value="chain_of_custody_gap">{language === "fr" ? "Rupture de traçabilité" : "Chain of Custody Gap"}</SelectItem>
                                    <SelectItem value="other">{language === "fr" ? "Autre motif" : "Other"}</SelectItem>
                                  </SelectContent>
                                </Select>
                              </div>
                              ...
                              {reasonCode === "missing_document" && (
                                <div className="space-y-2">
                                  <Label className="text-sm">{language === "fr" ? "Documents à remplacer" : "Documents to replace"}</Label>
                                  <div className="space-y-1.5 rounded-lg border p-3">
                                    {Object.entries(MANIFEST_DOC_LABELS).map(([type, label]) => (
                                      <div key={type} className="flex items-center gap-2">
                                        <Checkbox
                                          id={`failed-${type}`}
                                          checked={failedDocTypesSelected.includes(type)}
                                          onCheckedChange={(checked) =>
                                            setFailedDocTypesSelected((prev) =>
                                              checked ? [...prev, type] : prev.filter((t) => t !== type)
                                            )
                                          }
                                        />
                                        <Label htmlFor={`failed-${type}`} className="text-xs font-normal cursor-pointer">{label}</Label>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                              ...
                              <div className="space-y-2">
                                <Label htmlFor="publicNotes" className="text-sm">
                                  {language === "fr" ? "Note publique (visible par la contrepartie)" : "Public note (visible to counterparty)"}
                                </Label>
                                <Textarea
                                  id="publicNotes"
                                  value={publicNotes}
                                  onChange={(e) => setPublicNotes(e.target.value)}
                                  placeholder={language === "fr" ? "Observations communiquées à la contrepartie…" : "Observations shared with counterparty…"}
                                  rows={3}
                                />
                              </div>
                              ...
                              <div className="space-y-2">
                                <Label htmlFor="internalNotes" className="text-sm text-muted-foreground">
                                  {language === "fr" ? "Note interne (non transmise)" : "Internal note (not shared)"}
                                </Label>
                                <Textarea
                                  id="internalNotes"
                                  value={internalNotes}
                                  onChange={(e) => setInternalNotes(e.target.value)}
                                  placeholder={language === "fr" ? "Observations internes à la Banque Centrale…" : "Internal observations for Banque Centrale…"}
                                  rows={2}
                                  className="text-xs"
                                />
                              </div>
                            ...
                            <CardFooter className="flex flex-col gap-2">
                              ...
                              >
                                ...
                                {reviewSubmitting === "validate"
                                  ...
                                  : (language === "fr" ? "Valider le manifeste" : "Validate Manifest")}
                              </Button>
                              <Button
                                variant="outline"
                              ...
                              >
                                ...
                                {reviewSubmitting === "return"
                                  ? (language === "fr" ? "Retour en cours…" : "Returning…")
                                  : (language === "fr" ? "Retourner à la contrepartie" : "Return to Counterparty")}
                              </Button>
                            </CardFooter>
                          </Card>
                        )}
                        ...
                        {(manifest.status === "accepted" || manifest.status === "returned") && (
                          <Card>
                            <CardHeader className="pb-3">
                              <CardTitle className="text-sm">{language === "fr" ? "Décision de révision" : "Review Decision"}</CardTitle>
                            </CardHeader>
                              ...
                              {manifest.reason_code && (
                                <div className="flex items-center gap-2">
                                  <Badge variant="outline" className="text-xs">
                                    {{
                                      missing_document: language === "fr" ? "Document manquant" : "Missing Document",
                                      weight_discrepancy: language === "fr" ? "Écart de poids" : "Weight Discrepancy",
                                      chain_of_custody_gap: language === "fr" ? "Rupture de traçabilité" : "Chain of Custody Gap",
                                      other: language === "fr" ? "Autre" : "Other",
                                    }[manifest.reason_code as string] ?? manifest.reason_code}
                                  </Badge>
                                </div>
                              )}
                              {manifest.review_notes && (
                                <p className="text-sm text-muted-foreground">{manifest.review_notes}</p>
                              )}
                              {!manifest.reason_code && !manifest.review_notes && (
                                <p className="text-sm text-muted-foreground italic">{language === "fr" ? "Aucune note." : "No notes."}</p>
                              )}
                            </CardContent>
                          </Card>
                        )}
